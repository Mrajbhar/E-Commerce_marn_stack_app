...
const HomePage = () => {
  ...
  const getSpecialProducts = async () => {
    try {
      // Fetch the first two pages once, then derive three DIFFERENT sets
      // so New Arrivals / Best Sellers / Popular don't all show identical items.
      const [p1, p2] = await Promise.all([
        axios.get(`${process.env.REACT_APP_API}/api/v1/product/product-list/1`),
        axios.get(`${process.env.REACT_APP_API}/api/v1/product/product-list/2`),
      ]);
      const all = [...(p1.data.products || []), ...(p2.data.products || [])];
      const unique = Array.from(new Map(all.map((x) => [x._id, x])).values());

      // New arrivals = newest first (API returns newest-first)
      setNewArrivals(unique.slice(0, 8));

      // Best sellers = highest rated / most reviewed
      const byRating = [...unique].sort(
        (a, b) =>
          (b?.ratings?.average || 0) - (a?.ratings?.average || 0) ||
          (b?.ratings?.count || 0) - (a?.ratings?.count || 0),
      );
      setBestSellers(byRating.slice(0, 8));

      // Popular = a different slice so it isn't a repeat of New Arrivals
      const rest = unique.slice(8, 16);
      setPopularProducts(rest.length ? rest : unique.slice(0, 8));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  // newest real product for the intro card
  // Auto-rotating featured product: changes once per day on its own.
  // Builds a de-duplicated pool from all loaded products, then picks one
  // based on the current day number so it cycles through the catalog
  // (day 1 -> item 0, day 2 -> item 1, ...). No backend job needed.
  ...
  const fDisc =
    featured?.originalPrice && featured.originalPrice > featured.price
      ? Math.round(
          ((featured.originalPrice - featured.price) / featured.originalPrice) *
            100,
        )
      : 0;
          ...
          <div className="home-intro-inner">
            <div className="home-intro-left">
              ...
              <div className="home-intro-stats">
                ...
                <div className="hi-stat">
                  <span className="hi-n">
                    {categories.length || bentoSource.length}
                  </span>
              ...
              {featured?._id ? (
                ...
                >
                  <div className="hero-product-top">
                    {fDisc > 0 && (
                      <span className="hero-product-badge">{fDisc}% OFF</span>
                    )}
                    {fRating > 0 && (
                      <span className="hero-product-rating">
                        ★ {fRating.toFixed(1)}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="hero-product-info">
                    <span className="hero-product-eyebrow">
                      Featured product
                    </span>
                    <div className="hero-product-line">
                      ...
                      <div className="hero-product-prices">
                        ...
                        {fDisc > 0 && (
                          <span className="hero-product-original">
                            {inr(featured.originalPrice)}
                          </span>
                        )}
                      </div>
                    </div>
